// Output options: 0 not in word, 1 in word, 2 correct position
function checkGuess(target, guess) {
    if (target.length !== 5 || guess.length !== 5) {
        console.log("Error: Word length wrong");
    }
    let remainingLetters = target;
    let positionValues = [0, 0, 0, 0, 0];
    
    for (let i = 0; i < 5; i++) {
        if (target.charAt(i)=== guess.charAt(i)) {
            positionValues[i] = 2;
            remainingLetters = remainingLetters.replace(guess.charAt(i), "");
            //remainingLetters = remainingLetters.substring(0, i) + remainingLetters.substring(i + 1);
        } else if ( remainingLetters.includes(guess.charAt(i)) ) {
            positionValues[i] = 1;
            remainingLetters = remainingLetters.replace(guess.charAt(i), "");
        }
    }
...
function tooManyLetters() {
    document.getElementById("warning").innerHTML = "Guess must be 5 letters";
}

function notRealWord() {
    document.getElementById("warning").innerHTML = "Guess must be real word";
}
...
let guessCount = 0;
console.log(checkGuess("scope", "scoop"))
let validCount = 15918;
let validGuessWords = `aahed
aalii
aargh
aaron
abaca
...
zunis
`;
let solutionCount = 2315;
let validSolutions = `aback
abase
abate
abbey
abbot
...
let solutionIndex = Math.floor(Math.random() * (solutionCount - 0 + 1) + 0);
let solution = validSolutions.slice(solutionIndex * 6, solutionIndex * 6 + 5).toUpperCase();
